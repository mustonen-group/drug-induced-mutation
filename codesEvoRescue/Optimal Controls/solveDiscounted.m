...
% Append the first multiplier dynamics (different Hamiltonian)
L1=@(t,lambda1,lambda2,S,R,u) -mu(t,u)*exp((Tf-t)*rr)-lambda1*(rs*(1-(2*S+R)/K)-d(t,u)-mu(t,u))-lambda2*(mu(t,u)-rr*R/K); % dlambda1/dt 
...
while(test < 0)
    ...
    for i=1:n
        ...
        fun=@(u) S(i)*(alfa*exp((Tf-T(i))*rr)-lambda1(i)*(dd(u)+alfa))+lambda2(i)*S(i)*alfa; % add lambda3 for isoperimetric constraint
        % Plot W to visualize the roots and adjust the root finding
        % algorithm if necessary
        W=S(i)*(alfa*exp((Tf-T(i))*rr)-lambda1(i)*(DD+alfa))+lambda2(i)*S(i)*alfa;
%         [x_end,ind]=min(abs(W));
%         Z2(i)=U(ind);
        [x_end,ind]=min(W);
...
subplot(1,2,1)
plot(T,dt*cumtrapz(S.*(m0+alfa*u).*exp(rr*(Tf-T))),'linewidth',3)
hold on
for j=1:length(Us)
    csts(j)=dt*trapz(Ss(j,:).*(m0+alfa*Us(j)*ones(1,n)).*exp(rr*(Tf-T)));
    plot(T,dt*cumtrapz(Ss(j,:).*(m0+alfa*Us(j)*ones(1,n)).*exp(rr*(Tf-T))), 'linewidth',3)
end
legend(leg)
